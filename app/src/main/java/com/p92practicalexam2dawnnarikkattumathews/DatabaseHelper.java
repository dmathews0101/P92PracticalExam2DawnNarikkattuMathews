...
public class DatabaseHelper extends SQLiteOpenHelper {
      ...
      public List<RestaurantCart> getRestaurantItems() {
  		SQLiteDatabase db = SQLiteDatabase.openDatabase(pathToSaveDBFile, null, SQLiteDatabase.OPEN_READONLY);
 		String query = "SELECT id, name, details, image, price FROM menu";
  		Cursor cursor = db.rawQuery(query, null);
  		List<RestaurantCart> list = new ArrayList<RestaurantCart>();
  		while(cursor.moveToNext()) {
	  		RestaurantCart resc = new RestaurantCart();
	  		resc.setItem_name(cursor.getString(1));
	  		resc.setItem_ingredients(cursor.getString(2));
	  		resc.setItem_price(cursor.getString(4));
	  		list.add(resc);
  		}
  		db.close();
  		return list;
